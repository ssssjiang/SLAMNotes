...
int main(int argc, char **argv) {
    ...
    cv::drawKeypoints(first_image, keypoints, image_show);
    ...
    // cv::drawMatches(first_image, keypoints, second_image, keypoints2, matches, image_matches);
...
void computeAngle(const cv::Mat &image, vector<cv::KeyPoint> &keypoints) {
	...
	// START YOUR CODE HERE (~7 lines)
        ...
        if(!(kp.pt.x < half_patch_size || kp.pt.y < half_patch_size)){
            double m01 = 0, m10 = 0;
            for(int ou = -half_patch_size; ou < half_patch_size; ou++){
                for(int ov = -half_patch_size; ov < half_patch_size; ov++){
                    m10 += kp.pt.x * image.at<uchar>(kp.pt.x, kp.pt.y);
                    m01 += kp.pt.y * image.at<uchar>(kp.pt.x, kp.pt.y);
                    kp.angle = std::atan(m01 / m10) * 180 / pi;
                }
            }
        }
        // END YOUR CODE HERE
...
void computeORBDesc(const cv::Mat &image, vector<cv::KeyPoint> &keypoints, vector<DescType> &desc) {
    for (auto &kp: keypoints) {
        ...
        for (int i = 0; i < 256; i++) {
            ...
            int up = ORB_pattern[i * 4], vp = ORB_pattern[i * 4 + 1], uq = ORB_pattern[i * 4 + 2], vq = ORB_pattern[i * 4 + 3];
            double ang = kp.angle * pi / 180;
            int up_h = up * std::cos(ang) - vp * std::sin(ang), vp_h = up * std::sin(ang) + vp * std::cos(ang);
            int uq_h = uq * std::cos(ang) - vq * std::sin(ang), vq_h = uq * std::sin(ang) + vq * std::cos(ang);
            if(0 <= up_h && up_h < image.rows && 0 <= vp_h && vp_h < image.cols && 0 <= uq_h && uq_h < image.rows && 0 <= vq_h && vq_h < image.cols)
               d[i] = image.at<uchar>(up_h, vp_h) > image.at<uchar>(uq_h, vq_h) ? 0 : 1;
            else d.clear();
	    // END YOUR CODE HERE
        }
        desc.push_back(d);
...
void bfMatch(const vector<DescType> &desc1, const vector<DescType> &desc2, vector<cv::DMatch> &matches) {
    ...
    for(int i = 0; i < desc1.size(); i++)
    {
        ...
        {
            DescType d1 = desc1[i], d2 = desc2[j];
            int d = 0;
            ...
            if(d <= d_max)
            {
                matches.push_back(cv::DMatch(i, j, d));
            }
        }
    }
    // END YOUR CODE HERE
